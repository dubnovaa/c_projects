...
int s21_tests(s21_decimal a, s21_decimal b) {
    int error = 0;
    for (int i = 0; i < 3; i++) {
...
// -4.5 => -4
START_TEST(round_3) {
  s21_decimal value = {{45, 0, 0, ((1 << 31) | (1 << 16))}};
  s21_decimal answer = {{4, 0, 0, (1 << 31)}};
  s21_decimal result;
  ck_assert_int_eq(0, s21_round(value, &result));
  int k = s21_tests(result, answer);
  ck_assert_int_eq(k, 0);
...
START_TEST(float_to_decimal1) {
  ...
  int k = s21_is_equal(result, answer);
...
START_TEST(float_to_decimal4) {
  s21_decimal result = {{0, 0, 0, 0}};
  s21_decimal answer = {{155556, 0, 0, ((1 << 31) | (5 << 16))}};
  int j = 0;
  j = s21_from_float_to_decimal((float)-1.55556, &result);
  int k = s21_is_equal(result, answer);
  ck_assert_int_eq(k, 1);
  ck_assert_int_eq(j, 0);
}
...
START_TEST(float_to_decimal5) {
  s21_decimal result = {{0, 0, 0, 0}};
  s21_decimal answer = {{1234567, 0, 0, ((1 << 31) | (4 << 16))}};
  int j = 0;
  j = s21_from_float_to_decimal((float)-123.4567, &result);
...
START_TEST(div_3) {
  ...
  s21_decimal result = {0};
  s21_decimal answer = {{26000000, 0, 0, (7 << 16)}};
  normalize_num(&answer);
  ck_assert_int_eq(0, s21_div(value_1, value_2, &result));
  int k = s21_tests(result, answer);
  ck_assert_int_eq(k, 0);
...
START_TEST(div_7) {
  ...
  s21_decimal result = {0};
  s21_decimal answer = {{26000000, 0, 0, ((1 << 31) | (7 << 16))}};
  normalize_num(&answer);
  ck_assert_int_eq(0, s21_div(value_1, value_2, &result));
  int k = s21_tests(result, answer);
  ck_assert_int_eq(k, 0);
...
START_TEST(decimal_to_int6) {
  s21_decimal answer = {0,0,0,0};
  int j = 0;
  j = s21_from_decimal_to_int(answer, NULL);
  // ck_assert_int_eq(a, b);
  ck_assert_int_eq(j, 1);
}
END_TEST
...
Suite *s21_decimal_tests_create() {
  {
    ...
    tcase_add_test(s21_decimal_tests, decimal_to_int6);
